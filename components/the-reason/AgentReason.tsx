...
/* ════════════════════════════════════════════════════════════════════════
   AGENT VIEW — "The Reason", machine-readable.
   The canonical agent rendering, shared verbatim by BOTH /reason and
   /the-reason when viewMode === 'agent'. The essay reads as clean semantic
   prose — one <h1>, then <hr>-separated chapters of <h2> + paragraphs, no
   imagery and no scroll choreography. Each block still fades in via <Reveal>
   (a pure-opacity CSS reveal, force-shown in agent mode by globals.css) so the
...
/* Emphasis span — mirrors the /reason `W` helper: holds key phrases at the
   primary text colour against the dimmer body copy. */
...
export default function AgentReason() {
  return (
    <div>

      {/* Hero */}
      <section style={{ paddingTop: 'clamp(120px, 18vh, 250px)', paddingBottom: 40 }}>
        <div className="section-w">
          <Reveal><h1 style={{ maxWidth: 700 }}>The Reason</h1></Reveal>
        </div>
      </section>
      ...
      <Reveal>
        <hr />
        <div className="section-w" style={{ paddingTop: 24, paddingBottom: 24 }}>
          <div style={{ maxWidth: 640 }}>
            {/* Three lines: each "I am not…" lands as its own beat so the
                rhetorical refusal builds before the affirmative body opens. */}
